...
const MATIERES = [
  ...
  { id:'IST', nom:'Initiation Scientifique', icone:'🔬', code:'IST',
    gradient:'from-fuchsia-500 to-pink-600', bg:'bg-fuchsia-50', border:'border-fuchsia-200', text:'text-fuchsia-800', dot:'bg-fuchsia-500', niveau:['CM1'] },
  { id:'VE', nom:'Vivre ensemble',        icone:'🤝', code:'VE',
    gradient:'from-orange-500 to-red-600',  bg:'bg-orange-50',  border:'border-orange-200',  text:'text-orange-800',  dot:'bg-orange-500', niveau:['CM1'] },
  { id:'VM', nom:'Vivre dans son milieu', icone:'🌍', code:'VM',
    gradient:'from-cyan-400 to-blue-500',   bg:'bg-cyan-50',    border:'border-cyan-200',    text:'text-cyan-800',    dot:'bg-cyan-500',   niveau:['CM1'] },
...
function FlashcardGE({ question, reponse, explication, index, total }) {
  const [revele, setRevele] = useState(false);
  return (
    <div className="rounded-2xl bg-gradient-to-br from-teal-500 to-cyan-700 p-1 mb-4 shadow-lg">
      <div className="bg-white rounded-xl p-5">
        ...
        {!revele ? (
          <button onClick={() => setRevele(true)}
            className="w-full py-3 rounded-xl text-white font-medium bg-gradient-to-r from-teal-500 to-cyan-700 text-sm">
...
function FlashcardHI({ question, reponse, explication, index, total }) {
  const [revele, setRevele] = useState(false);
  return (
    <div className="rounded-2xl bg-gradient-to-br from-purple-500 to-violet-700 p-1 mb-4 shadow-lg">
      <div className="bg-white rounded-xl p-5">
        <div className="flex justify-between text-xs text-gray-400 mb-3">
          <span>Révision {index + 1}/{total}</span>
        </div>
        ...
        {!revele ? (
          <button onClick={() => setRevele(true)}
            className="w-full py-3 rounded-xl text-white font-medium bg-gradient-to-r from-purple-500 to-violet-700 text-sm">
            👆 Appuie pour voir la réponse
          </button>
        ) : (
          <div>
...
function SectionRevisionsHiGe({ chapitres, matiere }) {
  ...
  const isVE = matiere.id === 'VE';
  const isVM = matiere.id === 'VM';
  ...
  if (chapitreActif) {
    return (
      <div>
        ...
        <h3 className="font-bold text-gray-800 mb-4">{isGE ? '🌍' : isIST ? '🔬' : isVE ? '🤝' : isVM ? '🌍' : '🏛️'} {chapitreActif.titre}</h3>
        {chargement ? (
  ...
  return (
    <div>
      ...
      {chapitres.map((ch) => isGE ? (
        <button key={ch._id} onClick={() => chargerFlashcards(ch)}
          className="w-full text-left flex items-center gap-3 p-4 rounded-xl mb-3 bg-gradient-to-r from-teal-500 to-cyan-700 text-white shadow">
        ...
        <button key={ch._id} onClick={() => chargerFlashcards(ch)}
          className="w-full text-left flex items-center gap-3 p-4 rounded-xl mb-3 bg-gradient-to-r from-amber-500 to-yellow-600 text-white shadow">
          <span className="text-2xl">🤝</span>
          <div>
            <p className="font-semibold text-sm">{ch.titre}</p>
            <p className="text-xs opacity-80">Appuie pour réviser</p>
        ...
        {isGE ? (
          <>
            <button onClick={() => setVue('cours')}
              className={`flex-1 py-2.5 rounded-xl font-semibold text-sm transition-all ${vue === 'cours' ? 'bg-gradient-to-r from-teal-500 to-cyan-700 text-white shadow' : 'bg-gray-100 text-gray-500'}`}>
              📖 Cours
            </button>
          ...
          <>
            <button onClick={() => setVue('cours')}
              className={`flex-1 py-2.5 rounded-xl font-semibold text-sm transition-all ${vue === 'cours' ? 'bg-gradient-to-r from-amber-500 to-yellow-600 text-white shadow' : 'bg-gray-100 text-gray-500'}`}>
              📖 Cours
            </button>
            <button onClick={() => setVue('revisions')}
              className={`flex-1 py-2.5 rounded-xl font-semibold text-sm transition-all ${vue === 'revisions' ? 'bg-gradient-to-r from-amber-500 to-yellow-600 text-white shadow' : 'bg-gray-100 text-gray-500'}`}>
              🔁 Révisions
            </button>
          </>
        ) : isVM ? (
          ...
          <>
            <button onClick={() => setVue('cours')}
              className={`flex-1 py-2.5 rounded-xl font-semibold text-sm transition-all ${vue === 'cours' ? 'bg-gradient-to-r from-purple-500 to-violet-700 text-white shadow' : 'bg-gray-100 text-gray-500'}`}>
              📖 Cours
            </button>
            <button onClick={() => setVue('revisions')}
...
export function AccueilEleve() {
  ...
  const afficherSectionsHiGe = matiereActive === 'HI' || matiereActive === 'GE' || matiereActive === 'SV' || matiereActive === 'IST' || matiereActive === 'VE' || matiereActive === 'VM';
      ...
      <AnimatePresence mode="wait">
        {matiereActive && matiereObj ? (
          <motion.div key="chapitres">
            ...
            {afficherSectionsFr ? (
              <VueChapitresFr
                ...
                chapitres={chapitres}
                isValide={isValide}
                isVerrouille={isVerrouilleFrSafe}
          ...
          <motion.div key="accueil" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}>
            ...
            {/* ── Automatismes : Tables de multiplication (CM1 / CM2 / 6ème) ── */}
            ...
            {['CM1','CM2','6eme'].includes(user?.niveau) && (
              <motion.button
                ...
                <div className="px-4 py-3.5 flex items-center gap-3">
                  ...
                  <div className="text-blue-400 group-hover:text-blue-600 group-hover:translate-x-0.5 transition-all flex-shrink-0">
                    <ChevronRight size={18} />
                  </div>
                </div>
              </motion.button>
