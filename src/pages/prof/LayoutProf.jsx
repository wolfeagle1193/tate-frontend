...
import { LayoutDashboard, Users, BookOpen, PlusCircle, Menu, LogOut, ChevronRight, CalendarCheck, TrendingUp, ListChecks, Bell } from 'lucide-react';
...
const NAV = [
  { path: '/prof',              icon: LayoutDashboard, label: 'Tableau de bord'    },
  { path: '/prof/mes-eleves',   icon: Users,           label: 'Mes élèves'         },
  { path: '/prof/mes-lecons',   icon: BookOpen,        label: 'Mes leçons'         },
  { path: '/prof/preparer',     icon: PlusCircle,      label: 'Préparer un cours'  },
  { path: '/prof/qcm',          icon: ListChecks,      label: 'QCM'                },
  { path: '/prof/suivi-eleves', icon: TrendingUp,      label: 'Suivi des élèves'   },
  { path: '/prof/reservations', icon: CalendarCheck,   label: 'Cours particuliers' },
];
    ...
    <div className="flex flex-col h-full">
      <div className="p-5 border-b border-tate-border">
        ...
        <div className="mt-3 flex items-center gap-2">
          <div className="w-8 h-8 rounded-full bg-savoir/20 flex items-center justify-center text-sm font-bold text-savoir">
            {user?.nom?.[0]}
          </div>
          <div>
            <p className="text-xs font-semibold text-tate-terre truncate max-w-[140px]">{user?.nom}</p>
            <p className="text-xs text-tate-terre/40">Professeur</p>
          </div>
        </div>
      </div>
...
export function LayoutProf({ children, titre, action }) {
